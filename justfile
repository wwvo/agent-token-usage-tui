...
release-prepare version:
    ...
    echo "Bumping tools/windsurf-exporter/package.json + package-lock.json to {{version}}…"
    # `npm version` updates both package.json AND package-lock.json in
    # one shot (the lock embeds the root package's version twice —
    # top-level + the root "" package entry — both get rewritten).
    # `--no-git-tag-version` stops npm from opening its own
    # auto-commit/auto-tag flow; we drive git ourselves below.
    # `--allow-same-version` turns a re-run into a no-op instead of an
    # error, helpful when retrying after a mid-flight failure.
    ( cd tools/windsurf-exporter && npm version {{version}} \
    ...
    git add Cargo.toml Cargo.lock CHANGELOG.md \
            tools/windsurf-exporter/package.json \
            tools/windsurf-exporter/package-lock.json
    git commit -m "🔖 chore(release): bump {{version}}" \
               -m "- Cargo.toml + Cargo.lock: version bump to {{version}}" \
               -m "- tools/windsurf-exporter/package{,-lock}.json: version bump to {{version}}" \
